...
Namespace SpringStyleItem
    Public Class MyDockedBarControlViewInfo
        ...
        Protected Overrides Function PreCalcMultiLineSize(ByVal cache As GraphicsCache, ByVal sourceObject As Object, ByVal AMaxBarWidth As Integer, ByRef ARestWidth As Integer) As ArrayList
            Dim rows As ArrayList = MyBase.PreCalcMultiLineSize(cache, sourceObject, AMaxBarWidth, ARestWidth)
            If Not (TypeOf sourceObject Is DockedBarControl) OrElse rows Is Nothing OrElse DirectCast(sourceObject, DockedBarControl).ClientSize = Size.Empty OrElse Manager.Controller Is Nothing OrElse Not (TypeOf Manager.Controller Is MyBarAndDockingController) Then
                Return rows
            End If

            For i As Integer = 0 To rows.Count - 1
                Dim rowViewInfo As BarControlRowViewInfo = DirectCast(rows(i), BarControlRowViewInfo)
                ...
                For j As Integer = 0 To rowViewInfo.Links.Count - 1
                    ...
                    If linkInfo.Link.Item Is GetSpringStyleItem(springItemBarIndex) Then
                        springLinkIndex = j
                    Else
